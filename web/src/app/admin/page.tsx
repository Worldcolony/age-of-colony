...
export default function AdminPage() {
  ...
  return (
    <div className="flex flex-col gap-3">
      <h1 className="hud-title text-[28px]">Admin</h1>
      <div className="glass signal-brand flex flex-col gap-3 p-4">
        <h2 className="hud-title text-[20px]">Backend</h2>
        <div className="flex gap-2">
          <span className={`rounded-md border px-3 py-1 text-xs font-bold ${tx ? "border-lime/40 text-lime" : "border-danger/40 text-danger"}`}>TXLine {tx ? "✓" : "✗"}</span>
          <span className={`rounded-md border px-3 py-1 text-xs font-bold ${or ? "border-lime/40 text-lime" : "border-danger/40 text-danger"}`}>OpenRouter {or ? "✓" : "✗"}</span>
        </div>
        {health && !or && <p className="text-xs text-ink-faint">Set OPENROUTER_API_KEY to start games / demo.</p>}
      </div>

      <div className="glass flex flex-col gap-3 p-4">
        <h2 className="hud-title text-[20px]">Quick actions</h2>
      ...
      <div className="flex items-center justify-between">
        <h2 className="hud-title text-[20px]">Recent fixtures</h2>
